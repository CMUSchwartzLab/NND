""" utils functions to support NND and analysis
...
def heuristic_learning_rate(B):
  
  learning_rate = min(np.std(B), np.mean(B))/10.0
...
def torch_mask_mse(B, B_prd, M):
  
  return torch.norm((B_prd-B)*M, 2)**2 / M.sum()
